...
export default function OnboardingWizard() {
  ...
  const [email, setEmail] = useState('fitava.martinez@gmail.com');
  ...
  // Postpartum Routing (PRD Section 5 & 15)
  ...
  const [fitnessLevel, setFitnessLevel] = useState<FitnessLevel>('beginner'); // PRD Section 5: Preselect Beginner
  ...
  // OTP Verification Architecture (PRD Section 4 & 7: flexible strategy)
  const [otpSent, setOtpSent] = useState(false);
  const [otpCode, setOtpCode] = useState('');
  const [otpVerified, setOtpVerified] = useState(true); // AUTO_ACCEPT mode active by default
  ...
  const handleComplete = async () => {
    ...
    const planType = isPostpartum ? ('postpartum' as const) : bmiCategory === 'underweight' ? ('underweight' as const) : bmiCategory === 'overweight' || bmiCategory === 'obese' ? ('overweight' as const) : ('normal_weight' as const);
    ...
    // Save to local cache for offline agility
    store.currentUser = newUser;
    store.currentPlan = newPlan;
    saveLocalStore(store);

    // Persist to Supabase when environment credentials are ready
    ...
    if (supabase) {
      try {
        ...
        if (authData.user) {
          ...
          // Insert initial baseline BMI record
        ...
        console.error('Supabase onboarding sync note:', err);
      }
    }
    ...
    <div className="min-h-[100dvh] flex flex-col justify-between bg-[#0A0705] text-[#FAF8F5] p-6 max-w-md mx-auto relative">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-xs font-semibold text-[#8A8279]">
            Step {step} of 6
          </div>
        </div>

        {/* STEP 1: ACCOUNT & OTP (PRD Section 4: Internal OTP Verification) */}
        {step === 1 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white tracking-tight">Create your Athlete Account</h2>
            <p className="text-xs text-[#A8A096]">
              Sign up with your credentials to unlock your personalized fitness plan and daily habit tracking.
            </p>
            ...
            <div className="space-y-3 pt-2">
              ...
              {/* Seamless OTP hook reserved for when EMAIL_OTP strategy is activated */}
              {otpSent && !otpVerified && (
                <div className="bg-[#1E1914] border border-[#E37210]/30 rounded-2xl p-4 mt-3 space-y-3">
                  <div className="flex items-center space-x-2 text-xs text-[#FDBA74]">
                    <ShieldCheck className="w-4 h-4 text-[#E37210]" />
                    <span>Enter 4-digit code sent to your email</span>
                  </div>
                  <input
                    type="text"
                    placeholder="••••"
                    ...
                    maxLength={4}
                    className="w-full tracking-widest text-center text-lg font-bold bg-[#110D0A] border border-[#2A241E] rounded-xl py-2 text-white focus:outline-none focus:border-[#E37210]"
                  />
                </div>
              )}
            </div>
        ...
        {/* STEP 2: BODY METRICS & LIVE WHO BMI (PRD Section 5 & 6) */}
        {step === 2 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white tracking-tight">Your Body Metrics</h2>
            <p className="text-xs text-[#A8A096]">
              Adult BMI is screened using WHO standards (Weight ÷ Height²). Age and gender personalize safety rules.
            </p>
            ...
            {/* Ineligible Age Alert (PRD Section 6) */}
        ...
        {/* STEP 3: GOALS & TARGET WEIGHT WITH USER OVERRIDE (PRD Section 7) */}
        {step === 3 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white tracking-tight">Goals & Target Weight</h2>
            <p className="text-xs text-[#A8A096]">
              {goalRecommendation.reason} You have full freedom to override system suggestions.
            </p>
            ...
            <div className="space-y-2 pt-2">
              {[
                ...
                { id: 'recomposition', label: 'Improve Body Composition (Recomposition)' },
                { id: 'improve_fitness', label: 'General Cardiovascular & Daily Stamina' },
              ].map((g) => (
                <button
                  key={g.id}
            ...
            <div className="pt-3">
              <label className="text-xs text-[#C7BFB5] block mb-1">
                Suggested Target Weight (Healthy range: {healthyRange.minKg} - {healthyRange.maxKg} kg)
              </label>
              <input
                type="number"
        ...
        {/* STEP 4: SPECIAL PATHWAYS (POSTPARTUM ROUTING) (PRD Section 2 & 5) */}
        {step === 4 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white tracking-tight">Special Conditioning</h2>
            <p className="text-xs text-[#A8A096]">
              FitPocket tailors gentle pelvic floor and core rebuilding pathways when postpartum is selected.
            </p>
            ...
            {gender === 'female' ? (
                ...
                <div className="bg-[#16120E] border border-[#2A241E] rounded-2xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`w-12 h-6 rounded-full transition-colors relative p-0.5 ${
                        isPostpartum ? 'bg-[#E37210]' : 'bg-[#2A241E]'
                      }`}
                    >
                      <div
                        className={`w-5 h-5 rounded-full bg-white transition-transform ${
                          isPostpartum ? 'translate-x-6' : 'translate-x-0'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  {isPostpartum && (
                    <div className="space-y-3 pt-2 border-t border-[#2A241E]">
                      <div>
                        <label className="text-xs text-[#8A8279] block mb-1">Timeframe since delivery</label>
                        <select
                      ...
                      <div className="flex items-start space-x-2 pt-2">
                        ...
                        <label htmlFor="clearance" className="text-[11px] text-[#A8A096] leading-snug">
                          <strong>Medical Clearance Confirmation:</strong> I confirm that I have been cleared by my obstetrician/physician to engage in physical exercise.
                        </label>
                      </div>
                    </div>
                  )}
              ...
              <div className="bg-[#16120E] border border-[#2A241E] rounded-2xl p-4 text-xs text-[#A8A096]">
                No special conditions required for your profile. Click Continue to proceed to environment and schedule setup.
              </div>
            )}
          </div>
        )}

        {/* STEP 5: ENVIRONMENT & FITNESS LEVEL (PRD Section 8) */}
        {step === 5 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white tracking-tight">Workout Environment</h2>
            <p className="text-xs text-[#A8A096]">
              Choose where you perform your sessions. Beginner level is preselected for your safety.
            </p>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                ...
                className={`p-4 rounded-2xl border text-left transition-all ${
                  workoutEnvironment === 'home'
                    ? 'bg-[#E37210]/15 border-[#E37210]'
                    : 'bg-[#16120E] border-[#2A241E]'
                }`}
              >
                <Home className="w-6 h-6 text-[#E37210] mb-2" />
                <div className="text-sm font-semibold text-white">Home Workouts</div>
                <div className="text-[11px] text-[#8A8279]">No dedicated equipment; approved household items</div>
              </button>
              ...
              >
                ...
                <div className="text-[11px] text-[#8A8279]">Barbells, dumbbells, machines & cables</div>
              </button>
            </div>
        ...
        {/* STEP 6: SCHEDULE & SLEEP WINDOW (PRD Section 20) */}
        {step === 6 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white tracking-tight">Your Daily Rhythm</h2>
            <p className="text-xs text-[#A8A096]">
              Notifications strictly stop during sleep hours. Reminders recur every 30 mins while awake until completed.
            </p>
            ...
            <div className="bg-[#16120E] border border-[#2A241E] rounded-2xl p-4 text-xs text-[#8A8279] space-y-1 mt-2">
              <div className="font-semibold text-white">Plan Generation Summary:</div>
              <div>• Environment: {workoutEnvironment === 'home' ? 'Home Conditioning' : 'Gym Program'}</div>
              <div>• Level: {fitnessLevel.toUpperCase()}</div>
              <div>• BMI: {bmi} ({getBMILabel(bmiCategory)})</div>
              <div>• Daily hydration target: ~2.5 Liters</div>
            </div>
          </div>
        )}
      </div>
